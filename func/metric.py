...
def get_atten_entropy_score(scores,atten_for_output):
    entropy = 0.0
    avg_atten=sum(atten_for_output)/len(atten_for_output)
    for i,logits in enumerate(scores):
        if avg_atten<=atten_for_output[i]:
            conf = torch.max(logits.softmax(1)).cpu().item()
            entropy += conf*np.log(conf)*math.exp(atten_for_output[i])
    entropy = -1.0 * entropy
    return entropy

def get_entropy_simply_score(scores,atten_for_output):
    entropy = 0.0
    for i,logits in enumerate(scores):
        conf = torch.max(logits.softmax(1)).cpu().item()
        entropy += conf*np.log(conf)*math.exp(atten_for_output[i])
...
def get_entropy_simply_v2_score(scores,atten_for_output):
    entropy = 0.0
    for i,logits in enumerate(scores):
        conf = torch.max(logits.softmax(1)).cpu().item()
        entropy += conf*np.log(conf)*math.exp(atten_for_output[i])
    entropy = -1.0 * entropy/math.exp(sum(atten_for_output))
    return entropy


def get_range_avg_score(scores):
    ranges=[]
    for logits in scores:
        values, indices = torch.topk(logits.softmax(1), k=2, dim=-1)
        range_top2=torch.abs(torch.abs(values[0, 0] - values[0, 1])).cpu().item()
