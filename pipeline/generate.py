...
logInfo = open("./data/output/logInfo_{}_{}_{}.txt".format(args.model, args.dataset,args.project_ind), mode="w",encoding="utf-8")
...
def get_generations(model_name:str, args, seed=1, old_sequences=None, max_num_gen_once=args.num_generations_per_prompt):
    ...
    SenSimModel = SentenceTransformer('./data/weights/nli-roberta-large', device="cuda:3")
    bertscore = BERTScore(model_name_or_path="./data/weights/bert-base/", device="cuda:3")
    ...
    for batch_idx, batch in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        if args.decoding_method == 'beam_search':
            ...
            atten = model(dict_outputs.sequences,output_attentions=True).attentions[-1][0]
            solver = "max"
            if solver == "max": 
            ...
            atten_for_output=mean_atten[input_length:].cpu().tolist()
            entropy_simply=get_entropy_simply_score(scores,atten_for_output)
            range_avg=get_range_avg_score(scores)
            atten_entropy = get_atten_entropy_score(scores,atten_for_output)
            entropy_simply_v2=get_entropy_simply_v2_score(scores,atten_for_output)
        ...
        curr_seq.update(
            dict(
                atten_entropy=atten_entropy
            )
        )
        curr_seq.update(
            dict(
                entropy_simply=entropy_simply
            )
        )
        curr_seq.update(
            dict(
                entropy_simply_v2=entropy_simply_v2
            )
        )
        curr_seq.update(
            dict(
        ...
        print("entropy_simply: ",entropy_simply)
        print("atten_entropy: ",atten_entropy)
        print("entropy_simply_v2: ",entropy_simply_v2)
        ...
        print("range_avg: ",range_avg,file=logInfo)
        print("entropy_simply: ",entropy_simply,file=logInfo)
        print("atten_entropy: ",atten_entropy,file=logInfo)
        print("entropy_simply_v2: ",entropy_simply_v2,file=logInfo)
        print("\n","\n","\n", file=logInfo)
    return sequences
